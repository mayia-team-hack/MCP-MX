...
import { buildA2UIResource } from '../core/a2ui';
import {
  ALLOWED_VIEWS,
  MCP_MX_A2UI_VERSION,
...
function buildMapHtml(title: string, rows: Record<string, unknown>[]): string {
  const body = rows
    .slice(0, 50)
    .map((row) => {
      const label = escapeHtml(row['label'] ?? row['name'] ?? row['colonia'] ?? row['alcaldia'] ?? 'Punto');
...
export function register(server: McpServer): void {
  ...
  registerTool(
    ...
    async (args: ToolArgs) => {
      ...
      try {
        const uri = `a2ui://${vista}`;
        const resource = buildA2UIResource(
          uri,
          buildA2UIPayload(vista, titulo, filas, surface_id),
        );
        ...
        return {
          content: [
            {
              type: 'text',
              text: `Renderizando vista A2UI aprobada: ${vista}`,
            },
            resource,
          ],
        };
